...
let pcdFiles:any[]=[], topologyFiles:any[]=[];
...
function handle(m:J){
  ...
  if(m.type==='resource.catalog'){
    pcdFiles=m.pcds||[];topologyFiles=m.topologies||[];
  ...
  if(m.type==='topology.snapshot'||m.type==='topology.saved'){
    ...
    undoStack.length=0;redoStack.length=0;selected=null;edgeStart=null;
    renderTopology();renderResourceCatalog();setDirty();return;
  }
...
function edgeObject(a:number[],b:number[],id:string){
  ...
  const mesh=new THREE.Mesh(new THREE.CylinderGeometry(.025,.025,Math.max(len,.001),8),new THREE.MeshBasicMaterial({color:0x3ab4bb}));
  ...
  mesh.userData={kind:'edge',id,baseColor:0x3ab4bb};return mesh;
}
...
function renderInspector(){
  const none=$('emptyInspector'),vf=$<HTMLFormElement>('vertexForm'),ef=$<HTMLFormElement>('edgeForm');none.hidden=!!selected;vf.hidden=selected?.kind!=='vertex';ef.hidden=selected?.kind!=='edge';if(!selected||!topology)return;
...
$('fitView').onclick=()=>{const target=(topoGroup.visible&&topoGroup.children.length)?topoGroup:mapGroup;const box=new THREE.Box3().setFromObject(target);if(box.isEmpty())return;setRobotFollow(false);const size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3());controls.target.copy(center);camera.position.copy(center).add(new THREE.Vector3(size.length()*.55,-size.length()*.55,size.length()*.45));camera.near=Math.max(.01,size.length()/10000);camera.far=Math.max(100,size.length()*10);camera.updateProjectionMatrix()};
