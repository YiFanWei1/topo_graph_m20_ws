...
int main(int argc, char ** argv) {
  ...
  std::vector<char> clearance_ok(candidates->size(), 0);
  ...
  const auto safely_reachable = growRegion(
    candidates, candidate_tree, trajectory, &clearance_ok, cfg, safe_matched_seeds);

  auto initial_region = selectCloud(candidates, [&](std::size_t i) {return initially_reachable[i];});
  auto traversable_geometric = selectCloud(
  ...
  *continuous_raw += *route_ribbon;
  ...
  corridor_geometric->reserve(traversable_geometric->size() / 4);
  for (const auto & point : *traversable_geometric) {
