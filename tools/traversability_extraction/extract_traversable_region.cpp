...
struct Config {
  ...
  float strict_clearance_min_z = 0.40F;
  float strict_clearance_height = 0.41F;
...
int main(int argc, char ** argv) {
  ...
  const auto has_clearance = [&] (
    const Point & ground, const float minimum_height, const float maximum_height) {
    nearby.clear();
    ...
    for (const int map_id : nearby) {
      ...
      if (dxy > cfg.clearance_radius || dz < minimum_height || dz > maximum_height) {
        continue;
      }
  ...
  for (std::size_t candidate_id = 0; candidate_id < candidates->size(); ++candidate_id) {
    ...
    const bool original_has_clearance = has_clearance(
      ground, cfg.clearance_min_z, cfg.clearance_height);
    const bool strict_has_clearance = has_clearance(
      ground, cfg.strict_clearance_min_z, cfg.strict_clearance_height);
    if (original_has_clearance) {
  ...
  // This branch deliberately applies no height-range clearance test.  Keep it
  // as a separate artifact so the runtime YAML switch can genuinely disable
  // the feature without re-running extraction.
  Cloud::Ptr unfiltered_raw(new Cloud);
  *unfiltered_raw = *initial_region;
  *unfiltered_raw += *original_route_ribbon;
  Cloud::Ptr unfiltered_traversable(new Cloud);
  pcl::VoxelGrid<Point> unfiltered_voxel;
  unfiltered_voxel.setInputCloud(unfiltered_raw);
  unfiltered_voxel.setLeafSize(0.05F, 0.05F, 0.05F);
  unfiltered_voxel.filter(*unfiltered_traversable);
  saveCloud(output_dir / "traversable_surface_unfiltered.pcd", *unfiltered_traversable);
  ...
  for (const auto & point : *original_route_ribbon) {
    if (has_clearance(point, cfg.strict_clearance_min_z, cfg.strict_clearance_height)) {
  ...
  *corridor_raw += *strict_route_ribbon;
  ...
  summary << "{\n"
          ...
          << "  \"unfiltered_traversable_surface_points\": "
          << unfiltered_traversable->size() << ",\n"
          ...
          << "    \"strict_clearance_min_z_m\": " << cfg.strict_clearance_min_z << ",\n"
          << "    \"strict_clearance_height_m\": " << cfg.strict_clearance_height << ",\n"
  ...
  std::cout << "input=" << full->size()
            ...
            << " unfiltered=" << unfiltered_traversable->size()
