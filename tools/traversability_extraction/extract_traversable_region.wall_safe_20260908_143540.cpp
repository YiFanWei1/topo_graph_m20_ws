...
struct Config {
  ...
  // Synthetic route-ribbon points are retained only when a clearance-safe,
  // map-derived surface candidate exists nearby.  This keeps the route useful
  // for bridging sparse stairs without allowing an unsupported band through a wall.
  float route_support_radius = 0.18F;
...
int main(int argc, char ** argv) {
  ...
  const auto safely_reachable = growRegion(
    candidates, candidate_tree, trajectory, &geometric_clearance_ok, cfg, safe_matched_seeds);
  ...
  *combined_geometric_raw += *route_seeded_geometric;
  ...
  auto route_ribbon_raw = buildRouteSupportedRibbon(trajectory, cfg);
  saveCloud(output_dir / "route_supported_surface_raw.pcd", *route_ribbon_raw);

  // A route point is an aid to continuity, not evidence that empty space is
  // ground.  Require every synthetic point to have nearby map-derived support
  // that also passed the full clearance test.  Keep the interpolated route Z so
  // sparse stair treads can still be connected smoothly.
  auto clearance_safe_candidates = selectCloud(
    candidates, [&](std::size_t i) {return geometric_clearance_ok[i];});
  pcl::KdTreeFLANN<Point> clearance_safe_tree;
  clearance_safe_tree.setInputCloud(clearance_safe_candidates);
  Cloud::Ptr route_ribbon(new Cloud);
  Cloud::Ptr rejected_route_ribbon(new Cloud);
  route_ribbon->reserve(route_ribbon_raw->size());
  rejected_route_ribbon->reserve(route_ribbon_raw->size());
  std::vector<int> support_index(1);
  std::vector<float> support_distance(1);
  const float route_support_radius_squared =
    cfg.route_support_radius * cfg.route_support_radius;
  for (const auto & point : *route_ribbon_raw) {
    if (clearance_safe_tree.nearestKSearch(point, 1, support_index, support_distance) > 0 &&
        support_distance[0] <= route_support_radius_squared) {
      route_ribbon->push_back(point);
    } else {
      rejected_route_ribbon->push_back(point);
    }
  }
  route_ribbon->width = route_ribbon->size();
  route_ribbon->height = 1;
  route_ribbon->is_dense = true;
  ...
  saveCloud(output_dir / "route_unsupported_rejected.pcd", *rejected_route_ribbon);
  ...
  *corridor_raw += *route_ribbon;
  ...
  summary << "{\n"
          ...
          << "  \"route_supported_surface_raw_points\": " << route_ribbon_raw->size() << ",\n"
          << "  \"route_supported_surface_points\": " << route_ribbon->size() << ",\n"
          << "  \"route_unsupported_rejected_points\": "
          << route_ribbon_raw->size() - route_ribbon->size() << ",\n"
          ...
          << "    \"route_support_radius_m\": " << cfg.route_support_radius << ",\n"
